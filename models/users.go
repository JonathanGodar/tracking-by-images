...
type User struct {
	...
	PasswordHash string `boil:"password_hash" json:"-" toml:"-" yaml:"-"`
...
var UserRels = struct {
	...
	OwnerTrackers: "OwnerTrackers",
}

// userR is where relationships are stored.
type userR struct {
	OwnerTrackers TrackerSlice `boil:"OwnerTrackers" json:"OwnerTrackers" toml:"OwnerTrackers" yaml:"OwnerTrackers"`
}
...
func (r *userR) GetOwnerTrackers() TrackerSlice {
	if r == nil {
		return nil
	}
	return r.OwnerTrackers
}
...
// OwnerTrackers retrieves all the tracker's Trackers with an executor via owner_id column.
func (o *User) OwnerTrackers(mods ...qm.QueryMod) trackerQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"trackers\".\"owner_id\"=?", o.ID),
	)

	return Trackers(queryMods...)
}

// LoadOwnerTrackers allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadOwnerTrackers(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`trackers`),
		qm.WhereIn(`trackers.owner_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		object.R.OwnerTrackers = resultSlice
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.OwnerID {
				local.R.OwnerTrackers = append(local.R.OwnerTrackers, foreign)
				if foreign.R == nil {
					foreign.R = &trackerR{}
				}
...
// AddOwnerTrackers adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.OwnerTrackers.
// Sets related.R.Owner appropriately.
func (o *User) AddOwnerTrackers(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Tracker) error {
	var err error
	...
	if o.R == nil {
		o.R = &userR{
			OwnerTrackers: related,
		}
	} else {
		o.R.OwnerTrackers = append(o.R.OwnerTrackers, related...)
	}
